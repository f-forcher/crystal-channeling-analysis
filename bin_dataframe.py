...
def bin2D_dataframe(data, x_column, y_column, x_range, y_range, nxbins, nybins):
    ...
    xbincenters = np.linspace(low_xrange_edge + delta_xbins/2, high_xrange_edge + delta_xbins/2, num=nxbins + 1)[:-1]
    ybincenters = np.linspace(low_yrange_edge + delta_ybins/2, high_yrange_edge + delta_ybins/2, num=nybins + 1)[:-1]
